...
void matrix_init(buffer<float, 2>& buf) {
    ...
    host_accessor mt{ buf ,read_write };
...
void gauss_oneapi(buffer<float, 2>& buf, queue& q) {
    ...
    for (int k = 0; k < n; k++) {
       q.submit([&](handler& h) {
            ...
            h.parallel_for(range(n), [=](auto idx) {
                int j =idx;
                mt[k][j] = mt[k][j] / m[k][k];
       ...
       q.submit([&](handler& h) {
            accessor m{ buf, h, read_write };
            h.parallel_for(range(n , n ), [=](auto idx) {
                int i =idx.get_id(0);
                int j = idx.get_id(1);
                mt[i][j] = mt[i][j] - m[i][k] * mt[k][j];
                });
        ...
        q.submit([&](handler& h) {
            accessor m{ buf, h, read_write };
            h.parallel_for(range(0, n - (k + 1)), [=](auto idx) {
                int i = idx.get_id(0);
