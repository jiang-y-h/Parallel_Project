...
void block_run(int version) {
    ...
    if (version == 0) {
        ...
        f = &pipeline_block_gauss;
        inform = "pipeline block assign opt time is: ";
    }
...
void recycle_pipeline_gauss(int my_rank, int num_proc) {
    ...
    for (int k = 0; k < n; k++) {
        ...
        for (int i = k + my_rank; i < n; i += num_proc) {
            for (int j = k + 1; j < n; j++) {
                A[i][j] = A[i][j] - A[i][k] * A[k][j];
            }
            A[i][k] = 0.0;
...
void block2d_gauss() {
    ...
    if (my_rank == 0) {
        ...
        for (int i = 1; i < num_proc; i++) {
            int row = i / sqrt(i);
            int column = i % (int)sqrt(i);
            int block = n / sqrt(i);
            for (int j = column * block; j < column * block + block; j++)
                MPI_Send(A[row * block_size + j], block_size, MPI_FLOAT, i, 0, MPI_COMM_WORLD);
        }
        QueryPerformanceFrequency((LARGE_INTEGER*)&freq);
        QueryPerformanceCounter((LARGE_INTEGER*)&head);
        f(my_rank, num_proc);
...
void pipeline_block2d_gauss(int my_rank, int num_proc) {
    int block_size = n / sqrt(num_proc);
    int num = (int)sqrt(num_proc);
    int right_rank = (my_rank + 1) % num_proc;
    int down_rank = (my_rank + num) % num_proc;
    int left_rank = (my_rank - 1) % num_proc;
    int up_rank = (my_rank - num) % num_proc;
    int row_begin = block_size * (my_rank / num);
    int row_end = row_begin + block_size;
    int col_begin = block_size * (my_rank % num);
    int col_end = col_begin + block_size;
    for (int k = 0; k < n; k++) {
        if (k >= row_begin && k < row_end) {
            float ele;
            if (k >= col_begin && k < col_end) {
                ele = A[k][k];
                MPI_Send(&A[k][k], 1, MPI_FLOAT, right_rank, 2, MPI_COMM_WORLD);
                A[k][k] = 1.0;
            }
            else {
                MPI_Recv(&A[k][k], 1, MPI_FLOAT, left_rank, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                ele = A[k][k];
            }
            for (int j = col_begin; j < col_end; j++)
                A[k][j] = A[k][j] / ele;
            MPI_Send(A[k] + col_begin, block_size, MPI_FLOAT, down_rank, 3, MPI_COMM_WORLD);
        }
        else {
            if (k >= col_begin && k < col_end) {

                MPI_Recv(A[k] + col_begin, block_size, MPI_FLOAT, up_rank, 3, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                for (int i = row_begin; i < row_end; i++)
                    MPI_Send(A[i] + k, 1, MPI_FLOAT, right_rank, 4, MPI_COMM_WORLD);
                MPI_Send(A[k] + col_begin, block_size, MPI_FLOAT, down_rank, 2, MPI_COMM_WORLD);
            }
            else {
                for (int i = row_begin; i < row_end; i++)
                    MPI_Recv(A[i] + k, 1, MPI_FLOAT, left_rank, 4, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Recv(A[k] + col_begin, block_size, MPI_FLOAT, up_rank, 3, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Send(A[k] + col_begin, block_size, MPI_FLOAT, down_rank, 2, MPI_COMM_WORLD);
